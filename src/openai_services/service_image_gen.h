...
int			image_gen_init_service_data(void *options, void *ai_service, void *key);
int			image_gen_cleanup_service_data(void *ai_service);
void		image_gen_rest_transfer(void *ai_service);
void		image_gen_set_service_buffers(RestRequest * rest_request, RestResponse * rest_response,
										  ServiceData * service_data);
void		image_gen_post_header_maker(char *buffer, const size_t maxlen, const char *data,
										const size_t len);
int			image_gen_add_service_headers(CURL * curl, struct curl_slist **headers, void *service);
